...
func main() {
	...
	chrootBaseDir := flag.String("chroot-base-dir", filepath.Join("out", "vms-dst"), "`chroot` base directory")
	...
	netns := flag.String("netns", "ark1", "Network namespace to run Firecracker in")
				...
				func(di *protocol.DevInfo) storage.StorageProvider {
					...
					shards, err := modules.NewShardedStorage(
						...
						func(index, size int) (storage.StorageProvider, error) {
							return sources.NewFileStorageCreate(filepath.Join("out", fmt.Sprintf("%v-%v.bin", di.Name, index)), int64(size))
						},
					)
					if err != nil {
						panic(err)
